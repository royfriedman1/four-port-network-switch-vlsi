class checker_class extends component_base;
	...
	// Dump all leftover expected packets (per output queue)
	function void dump_leftovers(int outp, int max_to_print = 20);
		int n;
		n = expected_q[outp].size();

		$display("---- [%s] OUT%0d leftovers: %0d ----", pathname(), outp, n);

		for (int i = 0; i < n && i < max_to_print; i++) begin
			packet p = expected_q[outp][i];
			$display("  L%0d: SRC=%b TGT=%b DATA=%02h name=%s",
					 i, p.source, p.target, p.data, p.name);
		end

		if (n > max_to_print)
			$display("  ... %0d more not printed (increase max_to_print)", n - max_to_print);
	...
	function void final_check();
		for (int outp = 0; outp < NUM_PORTS; outp++) begin
			if (expected_q[outp].size() != 0) begin
				...
				// Print the actual leftover packets
				dump_leftovers(outp, /*max_to_print=*/50);
