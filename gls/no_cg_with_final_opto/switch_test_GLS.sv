...
// DUT Instantiation
// --------------------------------------------------------
switch_4port_wrapper u_switch_4port (
	.clk  (clk),
	.rst_n(rst_n),
	.ports(ports) 
);
...
initial begin
	...
	ports[0].valid_in  = 1'b0;
	ports[0].source_in = '0;
	ports[0].target_in = '0;
	ports[0].data_in   = '0;
	
	ports[1].valid_in  = 1'b0;
	ports[1].source_in = '0;
	ports[1].target_in = '0;
	ports[1].data_in   = '0;
	
	ports[2].valid_in  = 1'b0;
	ports[2].source_in = '0;
	ports[2].target_in = '0;
	ports[2].data_in   = '0;
	
	ports[3].valid_in  = 1'b0;
	ports[3].source_in = '0;
	ports[3].target_in = '0;
	ports[3].data_in   = '0;
...
always_ff @(posedge clk) begin
  ...
  if (rst_n && prev_rst_n) begin
	...
	/*
	st0 = u_switch_4port.u_gate.SWITCH_UNIT_MASTER_CONTROL_0__u_switch_port
			.u_Master_Control_unit.current_state;
	st1 = u_switch_4port.u_gate.SWITCH_UNIT_MASTER_CONTROL_1__u_switch_port
			.u_Master_Control_unit.current_state;
	st2 = u_switch_4port.u_gate.SWITCH_UNIT_MASTER_CONTROL_2__u_switch_port
			.u_Master_Control_unit.current_state;
	st3 = u_switch_4port.u_gate.SWITCH_UNIT_MASTER_CONTROL_3__u_switch_port
			.u_Master_Control_unit.current_state;

	*/

	st0 = u_switch_4port.u_gate.\SWITCH_UNIT_MASTER_CONTROL[0].u_switch_port
			.u_Master_Control_unit.current_state;
	st1 = u_switch_4port.u_gate.\SWITCH_UNIT_MASTER_CONTROL[1].u_switch_port
			.u_Master_Control_unit.current_state;
	st2 = u_switch_4port.u_gate.\SWITCH_UNIT_MASTER_CONTROL[2].u_switch_port
			.u_Master_Control_unit.current_state;
	st3 = u_switch_4port.u_gate.\SWITCH_UNIT_MASTER_CONTROL[3].u_switch_port
			.u_Master_Control_unit.current_state;
...
// PORT 0
// ----------------------------
assign dbg_if[0].wr_ptr =
  u_switch_4port.u_gate.\SWITCH_UNIT_MASTER_CONTROL[0].u_switch_port .u_FIFO.wr_ptr;

assign dbg_if[0].rd_ptr =
  u_switch_4port.u_gate.\SWITCH_UNIT_MASTER_CONTROL[0].u_switch_port .u_FIFO.rd_ptr;

assign dbg_if[0].full =
  ($clog2(4)'(dbg_if[0].wr_ptr + 2'd1) == dbg_if[0].rd_ptr);

assign dbg_if[0].wr_en =
  u_switch_4port.u_gate.\SWITCH_UNIT_MASTER_CONTROL[0].u_switch_port .u_FIFO.wr_en;

assign dbg_if[0].wr_accept = dbg_if[0].wr_en &&  ~dbg_if[0].full;


// ----------------------------
// PORT 1
// ----------------------------
assign dbg_if[1].wr_ptr =
  u_switch_4port.u_gate.\SWITCH_UNIT_MASTER_CONTROL[1].u_switch_port .u_FIFO.wr_ptr;

assign dbg_if[1].rd_ptr =
  u_switch_4port.u_gate.\SWITCH_UNIT_MASTER_CONTROL[1].u_switch_port .u_FIFO.rd_ptr;

assign dbg_if[1].full =
		($clog2(4)'(dbg_if[1].wr_ptr + 2'd1) == dbg_if[1].rd_ptr);

assign dbg_if[1].wr_en =
  u_switch_4port.u_gate.\SWITCH_UNIT_MASTER_CONTROL[1].u_switch_port .u_FIFO.wr_en;

assign dbg_if[1].wr_accept = dbg_if[1].wr_en && ~dbg_if[1].full;


// ----------------------------
// PORT 2
// ----------------------------
assign dbg_if[2].wr_ptr =
  u_switch_4port.u_gate.\SWITCH_UNIT_MASTER_CONTROL[2].u_switch_port .u_FIFO.wr_ptr;

assign dbg_if[2].rd_ptr =
  u_switch_4port.u_gate.\SWITCH_UNIT_MASTER_CONTROL[2].u_switch_port .u_FIFO.rd_ptr;

assign dbg_if[2].full =
		($clog2(4)'(dbg_if[2].wr_ptr + 2'd1) == dbg_if[2].rd_ptr);

assign dbg_if[2].wr_en =
  u_switch_4port.u_gate.\SWITCH_UNIT_MASTER_CONTROL[2].u_switch_port .u_FIFO.wr_en;

assign dbg_if[2].wr_accept = dbg_if[2].wr_en && ~dbg_if[2].full;


// ----------------------------
// PORT 3
// ----------------------------
assign dbg_if[3].wr_ptr =
  u_switch_4port.u_gate.\SWITCH_UNIT_MASTER_CONTROL[3].u_switch_port .u_FIFO.wr_ptr;

assign dbg_if[3].rd_ptr =
  u_switch_4port.u_gate.\SWITCH_UNIT_MASTER_CONTROL[3].u_switch_port .u_FIFO.rd_ptr;

assign dbg_if[3].full =
		($clog2(4)'(dbg_if[3].wr_ptr + 2'd1) == dbg_if[3].rd_ptr);

assign dbg_if[3].wr_en =
  u_switch_4port.u_gate.\SWITCH_UNIT_MASTER_CONTROL[3].u_switch_port .u_FIFO.wr_en;

assign dbg_if[3].wr_accept = dbg_if[3].wr_en && ~dbg_if[3].full;
